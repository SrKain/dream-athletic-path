...
| [`src/routes/athlete.$slug.tsx`](file:///c:/Users/kauan/OneDrive/%C3%81rea%20de%20Trabalho/dev%202.0/teamgo/dream-athletic-path/src/routes/athlete.$slug.tsx)                                     | Público             | **Perfil Público do Atleta**: Exibe bio, fotos, destaques, estatísticas de recrutamento, GPA, nível de inglês, conquistas para Coaches. Possui Hero cinematográfico com vídeo de apresentação do YouTube ao fundo sob máscara verde esmeralda, e seção dedicada de vídeos *In Court* e *Apresentação/Sobre* incorporados em 16:9 (`PublicYoutubePlayer`) logo abaixo do texto de bio. |
...
## Atualização 2026-08-19 — Correção e Embeds Diretos do YouTube no Perfil do Atleta (TASK-028)

- **Embed Direto e Imediato (`src/components/public-youtube-player.tsx`)**: O componente passou a renderizar a tag `<iframe>` nativa do YouTube imediatamente com `loading="lazy"`, `allowFullScreen`, controles completos e som, eliminando a dependência do clique na capa estática para inicialização.
- **Inclusão Abrangente de Vídeos (`src/lib/public-videos.ts`)**: `groupPublicVideos()` agora integra `profile.highlight_video_url` automaticamente à lista de vídeos caso não esteja presente, e expõe `allVideos` e a flag booleana `hasVideos`.
- **Seção Unificada de Vídeos no Perfil Público (`src/routes/athlete.$slug.tsx`)**: Renderiza a seção "In Court & Presentation Film" sempre que houver qualquer vídeo cadastrado (`hasVideos === true`), exibindo grid responsivo de 1 coluna em mobile e 2 em desktop com badges descritivos para cada categoria (`About & Goals`, `In Court`, `Highlight Reel`, `Featured Film`).
- **Migração SQL de Correção de RLS (`db/migrations/0011_athlete_videos_rls_fix.sql`)**: Garante que a política RLS pública de `athlete_videos` utilize a função security definer `public.athlete_is_public(athlete_id)`, prevenindo bloqueios de subquery em acessos de visitantes anônimos.
