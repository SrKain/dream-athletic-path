...
| **TASK-017** | 2026-08-16 19:54 | Usuário Humano | Antigravity AI | Correção da integração, salvamento e exibição de vídeos do YouTube no perfil do atleta (hero de fundo, highlights reels, apresentação, em quadra) e no catálogo público. | `[PENDENTE]` | Diagnóstico realizado: (1) Catálogo só buscava `kind = 'feature'`, ignorando highlights/apresentação/in_court/highlight_video_url; (2) Hero do perfil público não realizava fallback gracioso para outros vídeos cadastrados; (3) Admin bloqueava atualização de vídeos existentes do mesmo tipo em vez de substituir; (4) Falta de preview em tempo real e de salvamento unificado ao clicar em 'Salvar alterações'. Plano elaborado aguardando aprovação humana. |
