...
4. **Interações do Viewer**:
   ...
   - **Ações**:
     - _Recrutar_: Dispara WhatsApp com mensagem padrão parametrizada de `src/lib/contact.ts`.
     - _Compartilhar_: Web Share API nativa com fallback para área de transferência (Clipboard).
     - _Curtir_: Ícone de estrela (`Star`), com gravação persistente de likes reais no Supabase, atualização otimista instantânea e bloqueio contra spam/múltiplos cliques.
     - _Perfil_: Navegação direta para a rota pública da atleta (`/athlete/$slug`).
