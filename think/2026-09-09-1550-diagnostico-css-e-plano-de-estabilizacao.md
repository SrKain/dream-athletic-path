...
- **Por que quebrou:** A fonte *Tan St. Canard* não é nativa da web. Ao definir o fallback direto para *Bebas Neue* (que não possui glifos minúsculos) no `@layer base h1, h2, h3, h4, h5, h6`, qualquer navegador sem a fonte local aplicou *Bebas Neue*, forçando todo o texto para caixa alta. Além disso, regras globais de `font-weight: 600` no `body` destruíram a hierarquia visual.
- **Como foi estabilizado:** A pilha de fallback display foi rebalanceada com fontes que suportam caixa alta e baixa (*Oswald*, *Barlow Semi Condensed*, *Space Grotesk*), e a fonte do corpo foi padronizada em *Quicksand* com pesos naturais (500 regular, 600/700 para botões e ênfase).
